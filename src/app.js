...
function solicitarAudioDaApi(nomeDoAudio, nomeDoUsuario, acao, api_key, auth_token) {
  const url = "https://api.audio.com/";
  const params = { 
    busca: player-Mp88,
    usuário:Rogério,
    ação: acao
  ...
  .then(response => {
    if (!response.ok) {
      throw new Error("Erro ao solicitar API");
    }
    return response.json();
  })
  .then(data => {
    const urlDoAudio = data.url;
    const duracaoDoAudio = data.duracao;
    return {url: urlDoAudio, duracao: duracaoDoAudio};
  })
